			...
			if (mipLevels > 0 && mipLevels == (int)expectedMipLevels) {
				...
				for (int x = 0; x < mipLevels; x++) {
					...
					if (x >= mipStart) {
						if (x == mipStart) {
							// Base texture
							CreateTexture(fb->GetCommands(), mImage.get(), mipWidth, mipHeight, 4, fmt, pixelData + dataPos, expectedMipLevels - mipStart, false, mipSize);
						}
						else {
							// Mip
							CreateTextureMipMap(fb->GetCommands(), mImage.get(), x - mipStart, mipWidth, mipHeight, 4, fmt, pixelData + dataPos, mipSize);
						}
					}
